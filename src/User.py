...
class User():
    ...
    def __init__(self, user_id,speed,alpha):
        ...
        self.delta = 20 # db difference to disconnect the user
        self.state = 1
        self.ttt =0 # 100ms -> 5 times
    ...
    def PwrBSB(self):
        # Received Power From Base Station
        s = random.gauss(0, 4)
        self.BaseStationBPower =4.56 - 22*np.log10(5000-self.Location)+ s# + random
        ...
        while active:
            ...
            match self.state:
                case 1: #connected to BS-A
                    if self.BaseStationAPower - self.BaseStationBPower > self.delta: #drop user
                        self.disconnect()
                ...
                case 2: #connected to BS-B
                    if self.BaseStationBPower - self.BaseStationAPower > self.delta: #drop user
                        # print ("różnica mocy:    ", self.BaseStationBPower-self.BaseStationAPower)
                        self.disconnect()
